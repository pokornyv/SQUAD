...
from squadlib1 import FillEnergies, AndreevEnergy, GFresidues, SolveHF, FillGreenHF,KondoTemperature,check_params
...
if chat: print('# Kondo temperature (from Bethe ansatz): {0: .5e}'.format(float(KondoTemperature(U,GammaR+GammaL,eps))))
...
while any([sp.fabs(n-n_old)>p.P['ConvN'],sp.fabs(mu-mu_old)>p.P['ConvN']]):
	...
	if p.P['rootf'] == 'brentq':
		...
		mu = brentq(eqnA,p.P['MuMin'],p.P['MuMax'],xtol = p.P['ConvX'])	
...
SEnABS = sp.real(Sigman_F[int(ABSpos_F[0])])
SEaABS = sp.real(Sigmaa_F[int(ABSpos_F[0])])
