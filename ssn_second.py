...
N       = 2**p.P['M']-1			# number of points for bubble/SE fft calculation
...
try:
	[n,mu,ErrMsgHF] = ssn.SolveHFssn(params_F,chat)
except RuntimeError:
...
hfe = ed+U*n
[GFn_F,GFa_F] = ssn.FillGreenHF(params_F,hfe,mu,En_F)
...
.format(float(JCband),float(JCgap),float(JCband+JCgap)))
AC = ssn.AndreevConductance(GFa_F,En_F,params_F)
...
print('{0: .3f}\t{1: .3f}\t{2: .3f}\t{3: .3f}\t{4: .3f}\t{5: .3f}\t{6: .3f}\t{7: .8f}\t\
